...
class LoanEventDialog(tk.Toplevel):
    ...
    def _on_org_type_change(self) -> None:
        ...
        if org == period_lbl:
            self._rows["org_type"][0].grid()
            self._rows["org_type"][1].grid()
            self._rows["new_timing"][0].grid()
            self._rows["new_timing"][1].grid()
            self._rows["new_account"][0].grid_remove()
            self._rows["new_account"][1].grid_remove()
        elif org == account_lbl:
            self._rows["org_type"][0].grid()
            self._rows["org_type"][1].grid()
            self._rows["new_timing"][0].grid_remove()
            self._rows["new_timing"][1].grid_remove()
            self._rows["new_account"][0].grid()
            self._rows["new_account"][1].grid()
        else:
            self._rows["org_type"][0].grid()
            self._rows["org_type"][1].grid()
            self._rows["new_timing"][0].grid_remove()
